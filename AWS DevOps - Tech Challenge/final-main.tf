...
resource "aws_internet_gateway" "eshopig" {
  vpc_id = "${aws_vpc.eshopvpc.id}"
...
resource "aws_route_table" "public" {
  ...
  route {
    ...
    gateway_id = "${aws_internet_gateway.eshopig.id}"
  }
...
resource "aws_nat_gateway" "eshopvpc" {
...
resource "aws_route_table" "app" {
  ...
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = "${aws_nat_gateway.eshopvpc.id}"
  }
